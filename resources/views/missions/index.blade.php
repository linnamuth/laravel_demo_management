    ...
    <div class="row">
        <div class="col-lg-12 margin-tb mb-4">
            <div class="pull-left">
                <h4>Missions
                    <div class="float-end">
                        <a class="btn" style="background-color: #3b94f3;color:white"  href="{{ route('mission_requests.create') }}" class="btn btn-primary mb-3">Mission Request</a>
                    </div>
                </h4>
            </div>
        </div>
                    ...
                    @elseif (Auth::user()->department->name === 'sale department')
                        @if(Auth::user()->role->name === 'hr manager')
                            ...
                            @elseif(Auth::user()->role->name === 'cfo')
                                ...
                                @endif
                                @else
                                    @if($approvalStatus['hr manager'] === 'approved' && $approvalStatus['team leader'] === 'approved'&& $approvalStatus['cfo'] === 'approved' && $approvalStatus['ceo'] === 'approved' )
                                        <span style="color: green;">Approved</span>
                                     @elseif($approvalStatus['hr manager'] === 'rejected' && $approvalStatus['team leader'] === 'rejected'&& $approvalStatus['cfo'] === 'rejected' )
                                    <span style="color: red;">Rejected</span>
                                    @else
                                        <span style="color: orange;">Pending</span>
                                    @endif
                     ...
                     <td>
                        @if (Auth::user()->role->name === 'team leader' || Auth::user()->role->name === 'ceo' )
                            @if ($approvalStatus[Auth::user()->role->name] === 'pending')
                                <form action="{{ route('mission-leaves.approves', $mission->id) }}" method="POST" style="display: inline;">
                                    @csrf
                                    <button type="submit" class="btn btn-success btn-sm">Approve</button>
                                </form>
                                <form action="{{ route('mission-leaves.reject', $mission->id) }}" method="POST" style="display: inline;">
                                    @csrf
